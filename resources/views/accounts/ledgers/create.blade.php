...
<div class="container">
    ...
    <form action="{{ route('ledgers.store') }}" method="POST" class="mt-4">
        @csrf
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <input type="number" step="0.01" name="debit" id="debit" class="form-control @error('debit') is-invalid @enderror" value="{{ old('debit', 0) }}" required>
            @error('debit')
            <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <input type="number" step="0.01" name="credit" id="credit" class="form-control @error('credit') is-invalid @enderror" value="{{ old('credit', 0) }}" required>
            @error('credit')
            <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
