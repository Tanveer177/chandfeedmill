...
<div class="container">
    <h1>Edit Supplier</h1>
    <form action="{{ route('purchase-management.suppliers.update', $supplier) }}" method="POST" class="mt-4">
        @csrf
        @method('PUT')
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="company" id="company" class="form-control @error('company') is-invalid @enderror" value="{{ old('company', $supplier->company) }}">
            @error('company')
            <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
